...
{
    public class PaceExtractor : IFirmwareExtractor
    {
        public string RootFsType
        {
            get { return "LZMA CPIO"; }
        }
        ...
        public void ExtractParts(string imageFilename)
        {
            ...
            {
                using (FileStream kernelStream = File.Create(Path.Combine(extractDir, "vmlinux")))
                {
                    Misc.CopyStream(imageStream, kernelStream, 0x01A0, 0);

                    kernelStream.Seek(0, SeekOrigin.Begin);

                    // Now we want to try and find the lzma'd cpio filesystem

                    int cpioIndex = FileStringSearcher.FindInFile(kernelStream, Encoding.ASCII.GetString(new byte[] 
                    { 
                        0x5D, 0x00, 0x00, 0x00, 0x02, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF 
                    }));

                    if (cpioIndex != ~kernelStream.Length)
                    {
                        using (FileStream rootFsLzmaStream = File.Create(Path.Combine(extractDir, "filesystem.lzma")))
                        {
                            Misc.CopyStream(kernelStream, rootFsLzmaStream, cpioIndex, 0);
                        }
